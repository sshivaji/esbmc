...
std::string type_id_to_string(type2t::type_ids id)
{
  ...
  case type2t::signedbv_id:
    return "signed";
  default:
    return fmt::format("unknown({})", static_cast<int>(id));
  }
}

std::set<std::string> find_included_headers(
  const std::string &file_path,
  std::set<std::string> &processed_files)
{
  if (
    processed_files.find(file_path) != processed_files.end() ||
    is_system_path(file_path))
  {
    return {};
  }
  ...
  while (std::getline(source_file, line))
  {
    ...
    if (std::regex_search(line, match, include_pattern))
    {
      ...
      std::filesystem::path local_path =
        std::filesystem::path(file_path).parent_path() / header;
      if (std::filesystem::exists(local_path))
      {
        ...
        if (!is_system_path(resolved_path))
        {
          ...
          auto nested_headers =
            find_included_headers(resolved_path, processed_files);
    ...
    json serialize_impl(const expr2tc &expr)
    {
      ...
      try
      {
        if (is_pointer_type(expr->type))
        {
          ...
          {
            return {
              {"__type", "pointer"}, {"address", addr_str}, {"circular", true}};
          }
          ...
          return {
            {"__type", "pointer"}, {"address", addr_str}, {"value", nullptr}};
        }
        else if (is_struct_type(expr->type))
        {
          ...
          for (size_t i = 0; i < struct_type.members.size(); i++)
          {
            ...
            {
              expr2tc member_expr =
                member2tc(struct_type.members[i], expr, member_name);
