...
ESBMC can compile on ARM64 (M1/M2/M3/M4) Macs. However, it does not work reliably beyond compilation. You need to install the lastest MAC OS Dev tools. However, the compile option for GOTO_SYSROOT needs to be changed. Note that make -j8 can be increased to -j32 on faster Macs.
````
brew install z3
brew install bison
brew install clang
